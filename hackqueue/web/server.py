...
from hackqueue.db.models import Guild, utcnow
...
class WebServer:
    ...
    async def _load_board(self, guild_id: int, board_key: str, period: Period) -> Board | None:
        if board_key == "composite":
            return await self._boards.composite_board(guild_id, period)
        if board_key == CLAIMS_KEY:
            return await self._boards.claims_board(guild_id, period)
        try:
            platform = Platform(board_key)
        except ValueError:
            return None
        return await self._boards.platform_board(guild_id, platform, period)
